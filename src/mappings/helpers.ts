...
export function convertEosToDecimal(eos: BigInt): BigDecimal {
  return eos.toBigDecimal().div(exponentToBigDecimal(18))
}
...
export function isNullEosValue(value: string): boolean {
...
export function fetchTokenSymbol(tokenAddress: Address): string {
  ...
  if (symbolResult.reverted) {
    ...
    if (!symbolResultBytes.reverted) {
      ...
      if (!isNullEosValue(symbolResultBytes.value.toHexString())) {
...
export function fetchTokenName(tokenAddress: Address): string {
  ...
  if (nameResult.reverted) {
    ...
    if (!nameResultBytes.reverted) {
      ...
      if (!isNullEosValue(nameResultBytes.value.toHexString())) {
...
export function createLiquiditySnapshot(position: LiquidityPosition, event: ethereum.Event): void {
  ...
  snapshot.token0PriceUSD = token0.derivedEOS.times(bundle.eosPrice)
  snapshot.token1PriceUSD = token1.derivedEOS.times(bundle.eosPrice)
